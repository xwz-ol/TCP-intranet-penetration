...
func createControlChannel() {
	tcpListener, err := base.CreateTcpListenner(controlAddr)
	if err != nil {
		panic(err)
	}
...
func keepAlive() {
	go func() {
		for {
			...
			_, err := clientConn.Write(([]byte)(base.KeepAlive + "\n"))
			if err != nil {
...
func acceptUserRequest() {
	tcpListener, err := base.CreateTcpListenner(visitAddr)
	if err != nil {
		panic(err)
	}
	...
	for {
		...
		sendMessage(base.NewConnection + "\n")
	}
}
...
func acceptClientRequest() {
	tcpListener, err := base.CreateTcpListenner(tunnelAddr)
	if err != nil {
		panic(err)
	}
	defer tcpListener.Close()
...
func establishTunnel(tunnel *net.TCPConn) {
	...
	for key, connMatch := range connectionPool {
		if connMatch.accept != nil {
			go base.CopyConn(connMatch.accept, tunnel)
